...
from Timing_Trainer import Timing_Trainer
...
def main(args):
    ...
    trainer = Timing_Trainer.from_argparse_args(args)
    ...
    inference_time = trainer.validate(model = model, ckpt_path="data/vgg_conv.pth",dataloaders=dm.val_dataloader)
    end_time = time.time()


    print(f"Average input size: {None} byte, "
          f"Average running time: {(end_time-start_time) / inference_num}, "
          f"Average inference time: {inference_time/ inference_num} ")
...
if __name__ == '__main__':
    ...
    parser = Timing_Trainer.add_argparse_args(parser)
